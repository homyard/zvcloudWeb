...
(function(e) {
    var d = "bs_pagination";
    var b = { init: function(g) {
            var h = this;
            return this.each(function() {
                var C = h.data(d);
                if (typeof C == "undefined") {
                    var E = "3";
                    if (g.hasOwnProperty("bootstrap_version") && g.bootstrap_version == "2") { E = "2" }
                    var v = b.getDefaults.call(h, E);
                    C = e.extend({}, v, g) } else { C = e.extend({}, C, g) }
                if (C.totalRows == 0) { C.totalRows = C.totalPages * C.rowsPerPage }
                h.data(d, C);
                ...
                var y = h.attr("id"),
                    ...
                    w = "",
                    l, i, u, s, z, t, n, j, D;
                if (C.bootstrap_version == "3") { w += '<div class="' + C.mainWrapperClass + '">';
                    ...
                    w += "</div>";
                    w += "</div>";
                    if (C.showGoToPage && C.visiblePageLinks < C.totalPages) { w += '<div class="' + C.navGoToPageContainerClass + '">';
                        ...
                        w += '<input id="' + x + '" recordstype="text" class="' + C.navGoToPageClass + '" title="' + rsc_bs_pag.go_to_page_title + '">';
                        w += "</div>";
                        w += "</div>" }
                    if (C.showRowsPerPage) { w += '<div class="' + C.navRowsPerPageContainerClass + '">';
                        w += '<div class="input-group">';
                        ...
                        w += "</div>";
                        w += "</div>" }
                    if (C.showRowsInfo) { w += '<div class="' + C.navInfoContainerClass + '">';
                        w += '<div id="' + m + '" class="' + C.navInfoClass + '"></div>';
                        w += "</div>" } } else { w += '<div class="' + C.mainWrapperClass + '">';
                    w += '<div class="' + C.navListContainerClass + '">';
                    w += '<div class="' + C.navListWrapperClass + '">';
                    w += '<ul id="' + r + '" class="' + C.navListClass + '">';
                    w += "</ul>";
                    w += "</div>";
                    w += "</div>";
                    if ((C.showGoToPage && C.visiblePageLinks < C.totalPages) || C.showRowsPerPage) { w += '<div class="' + C.navInputsContainerClass + '">';
                        if (C.showGoToPage && C.visiblePageLinks < C.totalPages) { w += '<div class="' + C.navGoToPageWrapperClass + '">';
                            ...
                            w += "</div>" }
                        if (C.showRowsPerPage) { w += '<div class="' + C.navRowsPerPageWrapperClass + '">';
                ...
                if (!C.directURL) { u = "#" + k;
                    ...
                    h.off("click", z).on("click", z, function(I) {
                        ...
                        var G = C.currentPage;
                        f(y, H, G, false, true) }) }
                if (C.showGoToPage) { j = "#" + x;
                    h.off("keypress", j).on("keypress", j, function(I) {
                        if (I.which === 13) {
                            var H = parseInt(e(j).val());
                            e(j).val("");
                            if (!isNaN(H) && H > 0) {
                                if (H > C.totalPages) { H = C.totalPages }
                                var G = C.currentPage;
                                C.currentPage = H;
                                var F = H;
                                if (C.directURL) { location.href = C.directURL(F) } else { f(y, G, F, true, true) } }
                            I.preventDefault() } else {
                            if (!(I.which === 8 || I.which === 0 || (I.shiftKey === false && (I.which > 47 && I.which < 58)))) { I.preventDefault() } } }) }
                if (C.showRowsPerPage) { D = "#" + B;
                    h.off("keypress", D).on("keypress", D, function(H) {
                        if (H.which === 13) {
                            var I = parseInt(e(D).val());
                            if (!isNaN(I) && I > 0) {
                                if (I > C.maxRowsPerPage) { I = C.maxRowsPerPage }
                                e(D).val(I);
                                C.rowsPerPage = I;
            ...
            if (h == "2") { g.bootstrap_version = "2";
                ...
                g.navInfoClass = "" }
            return g }, getOption: function(g) {
            var h = this;
            return h.data(d)[g] }, getAllOptions: function() {
            var g = this;
            return g.data(d) }, destroy: function() {
            var g = this;
            g.removeData() }, setRowsInfo: function(g) {
            var h = this,
    ...
    var f = function(F, k, g, J, u) {
        ...
        if (J) {
            var l = c(B.nav_list_id_prefix, F),
                ...
                x, E, L, q, C, y = "",
                w = "javascript:void(0);";
            if (B.totalPages < B.visiblePageLinks) { z = 1;
                x = B.totalPages } else { C = Math.ceil(B.totalPages / B.visiblePageLinks);
                if (z > B.visiblePageLinks * (C - 1)) { z = B.totalPages - B.visiblePageLinks + 1 } else { L = z % B.visiblePageLinks;
                    q = L == 0 ? -B.visiblePageLinks + 1 : -L + 1;
                    z += q }
                x = z + B.visiblePageLinks - 1 }
            I.data("nav_start", z);
            I.data("nav_end", x);
            if (z > 1) { y = B.directURL ? B.directURL(1) : w;
            ...
            for (E = z; E <= x; E++) { y = B.directURL ? B.directURL(E) : w;
                G += '<li><a id="' + t + E + '" href="' + y + '">' + E + "</a></li>" }
            if (x < B.totalPages) { y = B.directURL ? B.directURL(x + 1) : w;
            ...
            v.html(G);
            if (B.disableTextSelectionInNavPane) { a(v) } }
        var p = e("#" + t + k),
            K = e("#" + t + g);
        ...
        if (B.showRowsInfo && B.showRowsDefaultInfo) {
            ...
            e("#" + j).html(A) }
        if (u) { I.triggerHandler("onChangePage", { currentPage: g, rowsPerPage: B.rowsPerPage }) } else { I.triggerHandler("onLoad", { currentPage: g, rowsPerPage: B.rowsPerPage }) } };
    var a = function(g) {
    ...
    e.fn.bs_pagination = function(h) {
        if (this.size() != 1) {
            ...
            e.error(g) }
        if (b[h]) {
            return b[h].apply(this, Array.prototype.slice.call(arguments, 1)) } else {
            if (typeof h === "object" || !h) {
                return b.init.apply(this, arguments) } else { e.error("Method " + h + " does not exist on jQuery." + d) } } } })(jQuery);
